...
import { AIConfig, AdminConfig, LiveSourceConfig } from './admin.types';
...
// 从环境变量解析直播源列表（LIVE_SOURCES 为 JSON 数组）
function getEnvLiveConfig(): LiveSourceConfig[] {
  const raw = process.env.LIVE_SOURCES;
  if (!raw) return [];
  try {
    const parsed = JSON.parse(raw);
    if (Array.isArray(parsed)) {
      return parsed.map((s: any, index: number) => ({
        key: s.key || `env-live-${index}`,
        name: s.name || s.key || `直播源${index + 1}`,
        url: s.url,
        ua: s.ua,
        epg: s.epg,
        from: 'config' as const,
        disabled: !!s.disabled,
      })).filter((s: LiveSourceConfig) => !!s.url);
    }
  } catch (e) {
    console.error('解析 LIVE_SOURCES 失败:', e);
  }
  return [];
}

// 从环境变量构建 AI 配置（管理后台配置优先，未配置时回退到环境变量）
function getEnvAIConfig(): AIConfig {
  return {
    Enabled: process.env.AI_ENABLED === 'true',
    CustomApiKey: process.env.AI_API_KEY || '',
    CustomBaseURL: process.env.AI_BASE_URL || 'https://api.openai.com/v1',
    CustomModel: process.env.AI_MODEL || 'gpt-4o-mini',
    Temperature: Number(process.env.AI_TEMPERATURE) || 0.7,
    MaxTokens: Number(process.env.AI_MAX_TOKENS) || 1000,
    EnableStreaming: process.env.AI_STREAMING !== 'false',
    EnableAIComments: process.env.AI_COMMENTS !== 'false',
    EnableWebSearch: process.env.AI_WEB_SEARCH === 'true',
    ...
    try {
      ...
      if (adminConfig) {
        ...
        adminConfig = {
          ...
          LiveConfig: getEnvLiveConfig(),
          AIConfig: getEnvAIConfig(),
        };
      }

      // 补充直播源与AI配置（管理后台优先，缺失时回退环境变量）
      if (!adminConfig.LiveConfig) {
        adminConfig.LiveConfig = getEnvLiveConfig();
      }
      if (!adminConfig.AIConfig) {
        adminConfig.AIConfig = getEnvAIConfig();
      }
